	...
	for (i = 0; i < num_sacks; i++) {
		...
		if (!tcp_is_sackblock_valid(tp, dup_sack,
			...
			if (dup_sack) {
				...
				printk(KERN_ERR "ADDLOG tcp_sacktag...");
			}
...
static void tcp_data_queue_ofo(struct sock *sk, struct sk_buff *skb)
{
	...
	if (unlikely(tcp_try_rmem_schedule(sk, skb, skb->truesize))) {
		printk(KERN_ERR "ADDLOG tcp_data_queue !!!");
...
static void tcp_data_queue(struct sock *sk, struct sk_buff *skb)
{
  ...
  printk(KERN_ERR "ADDLOG tcp_data_queue seq:%x,end_seq:%x,rcv:%x",TCP_SKB_CB(skb)->seq,TCP_SKB_CB(skb)->end_seq,tp->rcv_nxt);
...
queue_and_out:
		...
		if (!skb_queue_empty(&tp->out_of_order_queue)) {
			tcp_ofo_queue(sk);
			printk(KERN_ERR "ADDLOG tcp_data_queue2 rcv:%x",tp->rcv_nxt);
...
out_of_window:
		printk(KERN_ERR "ADDLOG tcp_data_queue3 out_of_window tp->rcv_nxt:%d",tp->rcv_nxt);
		tcp_enter_quickack_mode(sk);
		inet_csk_schedule_ack(sk);
drop:
		__kfree_skb(skb);
	...
	if (before(TCP_SKB_CB(skb)->seq, tp->rcv_nxt)) {
		...
		printk("ADDLOG partial packet: rcv_next %X seq %X - %X\n",
			   tp->rcv_nxt, TCP_SKB_CB(skb)->seq,
			   TCP_SKB_CB(skb)->end_seq);
				  ...
				  const struct tcphdr *th, int syn_inerr)
{
	...
	if (tcp_fast_parse_options(skb, th, tp) && tp->rx_opt.saw_tstamp &&
		...
		if (!th->rst) {
			...
			printk(KERN_ERR "ADDLOG tcp_validate_incoming PAWS");
			goto discard;
		}
	...
	if (!tcp_sequence(tp, TCP_SKB_CB(skb)->seq, TCP_SKB_CB(skb)->end_seq)) {
		...
		printk(KERN_ERR "ADDLOG tcp_validate_incoming seq!");
		goto discard;
	}
			...
			const struct tcphdr *th, unsigned int len)
{
	...
	if ((tcp_flag_word(th) & TCP_HP_BITS) == tp->pred_flags &&
		...
		if (tcp_header_len == sizeof(struct tcphdr) + TCPOLEN_TSTAMP_ALIGNED) {
			...
			if (!tcp_parse_aligned_timestamp(tp, th))
			{
				printk(KERN_ERR "ADDLOG tcp_rcv_established goto slowpath");
				goto slow_path;
			}
		...
		if (len <= tcp_header_len) {
			...
			if (len == tcp_header_len) {
				...
				tcp_ack(sk, skb, 0);
				__kfree_skb(skb);
				printk(KERN_ERR "ADDLOG tcp_validate_established begin to send data");
				tcp_data_snd_check(sk);
				return 0;
			} else { /* Header too small */
				TCP_INC_STATS_BH(sock_net(sk), TCP_MIB_INERRS);
				printk(KERN_ERR "ADDLOG tcp_validate_established header too small");
				goto discard;
			}
		} else {
			int eaten = 0;
			...
			if (tp->copied_seq == tp->rcv_nxt &&
				...
				if (eaten) {
					...
					tp->rcv_nxt = TCP_SKB_CB(skb)->end_seq;
					printk(KERN_ERR "ADDLOG tcp_validate_established tp->rcvnxt:%x",tp->rcv_nxt);
			...
			printk(KERN_ERR "ADDLOG tcp_validate_established tp->rcvnxt:%x,tp->rcv_wup:%d",tp->rcv_nxt,tp->rcv_wup);
...
slow_path:
	if (len < (th->doff << 2) || tcp_checksum_complete_user(sk, skb))
		goto csum_error;
	printk(KERN_ERR "ADDLOG tcp_validate_established slow_path...");
...
step5:
	...
	{
		printk(KERN_ERR "ADDLOG tcp_validate_established tcp_ack");
		goto discard;
	}
					 ...
					 const struct tcphdr *th, unsigned int len)
{
	...
	if (th->ack) {
		...
		if (!th->syn)
		{
			printk(KERN_ERR "ADDLOG tcp_validate_established !SYN");
			  ...
			  const struct tcphdr *th, unsigned int len)
{
	...
	if (!queued) {
discard:
	printk(KERN_ERR "ADDLOG tcp_rcv_state_process discard");
		__kfree_skb(skb);
	}
	return 0;
}
