...
printk(KERN_ERR "ADDLOG tcp_v4_err");
...
int tcp_v4_conn_request(struct sock *sk, struct sk_buff *skb)
{
	...
	return 0;
printk(KERN_ERR "ADDLOG tcp_v4_conn_request discard");
drop_and_release:
	dst_release(dst);
drop_and_free:
	reqsk_free(req);
...
int tcp_v4_do_rcv(struct sock *sk, struct sk_buff *skb)
{
	...
	{
		printk(KERN_ERR "ADDLOG tcp_v4_do_rcv 1");
		goto discard;
	}
#endif
...
csum_err:
	...
	printk(KERN_ERR "ADDLOG tcp_v4_do_rcv 2");
	goto discard;
}
...
bool tcp_prequeue(struct sock *sk, struct sk_buff *skb)
{
	...
	if (tp->ucopy.memory > sk->sk_rcvbuf) {
		...
		while ((skb1 = __skb_dequeue(&tp->ucopy.prequeue)) != NULL) {
			...
			printk(KERN_ERR "ADDLOG tcp_prequeue ");
		}
...
int tcp_v4_rcv(struct sk_buff *skb)
{
	...
	{
		printk(KERN_ERR "ADDLOG tcp_v4_rcv 1");
		goto discard_it;
	}
...
printk(KERN_ERR "ADDLOG tcp4_rcv seq:0x%x",TCP_SKB_CB(skb)->seq);
...
process:
	...
	if (unlikely(iph->ttl < inet_sk(sk)->min_ttl)) {
		NET_INC_STATS_BH(net, LINUX_MIB_TCPMINTTLDROP);
		printk(KERN_ERR "ADDLOG tcp_v4_do_rcv ttl");
		goto discard_and_relse;
	}

	if (!xfrm4_policy_check(sk, XFRM_POLICY_IN, skb))
		{
			printk(KERN_ERR "ADDLOG tcp_v4_do_rcv 3");
		goto discard_and_relse;
	}
	nf_reset(skb);
	...
	if (sk_filter(sk, skb))
		{
			printk(KERN_ERR "ADDLOG tcp_v4_do_rcv 4");
		goto discard_and_relse;
	}
	...
	if (!sock_owned_by_user(sk)) {
		...
		printk(KERN_ERR "ADDLOG tcp_v4_do_rcv 5");
		goto discard_and_relse;
	}
	bh_unlock_sock(sk);
...
no_tcp_socket:
	if (!xfrm4_policy_check(NULL, XFRM_POLICY_IN, skb))
		{
			printk(KERN_ERR "ADDLOG tcp_v4_do_rcv 6");
		goto discard_it;
	}
...
printk(KERN_ERR "ADDLOG tcp_v4_do_rcv  discard");
discard_it:
	/* Discard frame. */
	kfree_skb(skb);
	return 0;
...
do_time_wait:
	if (!xfrm4_policy_check(NULL, XFRM_POLICY_IN, skb)) {
		inet_twsk_put(inet_twsk(sk));
		printk(KERN_ERR "ADDLOG tcp_v4_do_rcv xfrm_policy_check");
		goto discard_it;
	}
	...
	printk(KERN_ERR "ADDLOG tcp_v4_do_rcv goto discard");
	goto discard_it;
}
