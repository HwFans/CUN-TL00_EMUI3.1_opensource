...
int sock_queue_rcv_skb(struct sock *sk, struct sk_buff *skb)
{
	...
	if (atomic_read(&sk->sk_rmem_alloc) >= sk->sk_rcvbuf) {
		printk(KERN_ERR "ADDLOG sock_queue_rcv_skb rcvbuf:%d",sk->sk_rcvbuf);
	...
	if (!sk_rmem_schedule(sk, skb, skb->truesize)) {
		printk(KERN_ERR "ADDLOG sock_queue_rcv_skb truesize:%d",skb->truesize);
		atomic_inc(&sk->sk_drops);
		return -ENOBUFS;
	}
	...
	printk(KERN_ERR "ADDLOG sock_queue_rcv_skb dropcount:%d",skb->dropcount);
...
int sk_receive_skb(struct sock *sk, struct sk_buff *skb, const int nested)
{
	...
	if (sk_filter(sk, skb))
	{
		printk(KERN_ERR "ADDLOG sk_receive_skb sk_filter");
	...
	if (sk_rcvqueues_full(sk, skb, sk->sk_rcvbuf)) {
		atomic_inc(&sk->sk_drops);
		printk(KERN_ERR "ADDLOG sk_receive_skb rcvqueue full rcvbuf:%d",sk->sk_rcvbuf);
		goto discard_and_relse;
	}
	if (nested)
	...
	if (!sock_owned_by_user(sk)) {
		...
		bh_unlock_sock(sk);
		atomic_inc(&sk->sk_drops);
		printk(KERN_ERR "ADDLOG sk_receive_skb rcvqueue full backlog rcvbuf:%d",sk->sk_rcvbuf);
		goto discard_and_relse;
	}
